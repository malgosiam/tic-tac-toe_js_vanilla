  ...
  registerEventListeners() {
    ...
    App.$.squares.forEach((square) => {
      square.addEventListener("click", (event) => {
        ...
        square.classList.add(["used-square"]);
        const turnLabel = document.createElement("p");
        turnLabel.innerText = `Player ${nextPlayer}, you're up!`;

        if (currentPlayer === 1) {
          squareIcon.textContent = "X";
          squareIcon.classList.add("yellow-color");

          turnIcon.textContent = "O";
          turnIcon.style.color = "var(--turquoise)";
        } else {
          squareIcon.textContent = "O";
          squareIcon.classList.add("turquoise-color");
